...
mpc_input_new_nstring (const char *filename, const char *string,
		       size_t length)
{
...
 mpc_parser_t * mpc_real (void)
{
    ...
    mpc_maybe_lift (mpc_and
		    (2, mpcf_strfold, mpc_char ('.'), mpc_digits (), free),
		    mpcf_ctor_str);
    ...
    mpc_expect (mpc_and (4, mpcf_strfold, p0, p1, p2, p3, free, free, free),
		"real");
...
mpcaf_grammar_string (mpc_val_t * x, void *s)
{
    ...
    (st->
     flags & MPCA_LANG_WHITESPACE_SENSITIVE) ? mpc_string (y) :
...
mpcaf_grammar_char (mpc_val_t * x, void *s)
{
    ...
    (st->
     flags & MPCA_LANG_WHITESPACE_SENSITIVE) ? mpc_char (y[0]) :
...
mpcaf_grammar_regex (mpc_val_t * x, void *s)
{
    ...
    (st->
     flags & MPCA_LANG_WHITESPACE_SENSITIVE) ? mpc_re (y) :
    mpc_tok (mpc_re (y));
				   ...
				   mpca_grammar_st_t * st)
{
  ...
   return (st->flags & MPCA_LANG_PREDICTIVE) ? mpc_predictive (r.
								 output) : r.
    output;
