 ...
 mpc_input_mark(mpc_input_t * i)
{
	...
	return;
	    ...
	    realloc(i->marks, sizeof(mpc_state_t) * i->marks_slots);
...
 static int mpc_input_range(mpc_input_t * i, char c, char d, char **o)
{
	...
	&& x <= d ? mpc_input_success(i, x, o) : mpc_input_failure(i, x);
...
 char *mpc_err_string(mpc_err_t * x)
{
			  ...
			  "%s:%i:%i: error: expected ", x->filename,
...
mpc_err_add_expected(mpc_input_t * i, mpc_err_t * x, char *expected)
{
	...
	mpc_malloc(i, strlen(expected) + 1);
				    ...
				    const char *prefix)
{
	...
	return NULL;
	    ...
	    mpc_malloc(i, strlen(prefix) + strlen(x->expected[0]) + 1);
...
 static mpc_val_t *mpc_parse_apply(mpc_input_t * i, mpc_apply_t f,
				     mpc_val_t * x)
{
...
mpc_parse_run(mpc_input_t * i, mpc_parser_t * p, mpc_result_t * r,
	      mpc_err_t ** e)
{
	...
	MPC_PRIMITIVE(mpc_input_char
		      (i, p->data.single.x, (char **) &r->output));
    case MPC_TYPE_RANGE:
	MPC_PRIMITIVE(mpc_input_range
		      (i, p->data.range.x, p->data.range.y,
		       (char **) &r->output));
    case MPC_TYPE_ONEOF:
	MPC_PRIMITIVE(mpc_input_oneof
		      (i, p->data.string.x, (char **) &r->output));
    case MPC_TYPE_NONEOF:
	MPC_PRIMITIVE(mpc_input_noneof
		      (i, p->data.string.x, (char **) &r->output));
    case MPC_TYPE_SATISFY:
	MPC_PRIMITIVE(mpc_input_satisfy
		      (i, p->data.satisfy.f, (char **) &r->output));
    case MPC_TYPE_STRING:
	MPC_PRIMITIVE(mpc_input_string
		      (i, p->data.string.x, (char **) &r->output));
    case MPC_TYPE_ANCHOR:
	MPC_PRIMITIVE(mpc_input_anchor
		      (i, p->data.anchor.f, (char **) &r->output));
	...
	MPC_SUCCESS(mpc_input_state_copy(i));
		...
		break;
			 ...
			 (i, results[j].error, p->data.repeat.n);
	    ...
	    : results_stk;
		     ...
		     (i, p->data.and.f, j, (mpc_val_t **) results);
...
mpc_parse_contents(const char *filename, mpc_parser_t * p,
		   mpc_result_t * r)
{
...
 mpc_parser_t * mpc_check_with(mpc_parser_t * a, mpc_check_with_t f,
				 void *x, const char *e)
{
...
 mpc_parser_t * mpc_checkf(mpc_parser_t * a, mpc_check_t f,
			     const char *fmt, ...)
{
...
 mpc_parser_t * mpc_check_withf(mpc_parser_t * a, mpc_check_with_t f,
...
{
    return
	mpc_expect(mpc_or(3, mpc_alpha(), mpc_digit(), mpc_underscore()),
		   "alphanumeric");
}
...
 mpc_parser_t * mpc_real(void)
{
	...
	mpc_maybe_lift(mpc_and
		       (2, mpcf_strfold, mpc_char('.'), mpc_digits(),
			free), mpcf_ctor_str);
    p30 = mpc_oneof("eE");
    p31 = mpc_maybe_lift(mpc_oneof("+-"), mpcf_ctor_str);
    p32 = mpc_digits();
    p3 =
	mpc_maybe_lift(mpc_and(3, mpcf_strfold, p30, p31, p32, free, free),
		       mpcf_ctor_str);
     return
	mpc_expect(mpc_and
		   (4, mpcf_strfold, p0, p1, p2, p3, free, free, free),
		   "real");
...
 mpc_parser_t * mpc_char_lit(void)
{
	...
	mpc_expect(mpc_between
		   (mpc_or(2, mpc_escape(), mpc_any()), free, "'", "'"),
		   "char");
...
 mpc_parser_t * mpc_string_lit(void)
{
	...
	mpc_expect(mpc_between
		   (mpc_many(mpcf_strfold, strchar), free, "\"", "\""),
		   "string");
...
 mpc_parser_t * mpc_regex_lit(void)
{
	...
	mpc_expect(mpc_between
		   (mpc_many(mpcf_strfold, regexchar), free, "/", "/"),
		   "regex");
...
{
    return mpc_and(3, mpcf_snd, mpc_soi(), a, mpc_eoi(), mpcf_dtor_null,
		   da);
}
...
 mpc_parser_t * mpc_re(const char *re)
{
		  ...
		  mpc_and(2, mpcf_re_or, Term,
			  mpc_maybe(mpc_and
				     (2, mpcf_snd_free, mpc_char('|'),
				      Regex, free)),
			  (mpc_dtor_t) mpc_delete ));
     mpc_define(Term, mpc_many(mpcf_re_and, Factor));
     mpc_define(Factor,
		  mpc_and(2, mpcf_re_repeat, Base,
			  mpc_or(5, mpc_char('*'), mpc_char('+'),
				  mpc_char('?'), mpc_brackets(mpc_int(),
							       free),
	...
	mpc_whole(mpc_predictive(Regex), (mpc_dtor_t) mpc_delete);
...
 mpc_val_t * mpcf_escape(mpc_val_t * x)
{
	...
	mpcf_escape_new(x, mpc_escape_input_c, mpc_escape_output_c);
...
 mpc_val_t * mpcf_escape_regex(mpc_val_t * x)
{
	...
	mpcf_escape_new(x, mpc_escape_input_raw_re,
...
 mpc_val_t * mpcf_unescape_regex(mpc_val_t * x)
{
	...
	mpcf_unescape_new(x, mpc_escape_input_raw_re,
			  mpc_escape_output_raw_re);
...
 mpc_val_t * mpcf_escape_string_raw(mpc_val_t * x)
{
	...
	mpcf_escape_new(x, mpc_escape_input_raw_cstr,
...
 mpc_val_t * mpcf_unescape_string_raw(mpc_val_t * x)
{
	...
	mpcf_unescape_new(x, mpc_escape_input_raw_cstr,
			  mpc_escape_output_raw_cstr);
...
 mpc_val_t * mpcf_escape_char_raw(mpc_val_t * x)
{
	...
	mpcf_escape_new(x, mpc_escape_input_raw_cchar,
...
 mpc_val_t * mpcf_unescape_char_raw(mpc_val_t * x)
{
	...
	mpcf_unescape_new(x, mpc_escape_input_raw_cchar,
			  mpc_escape_output_raw_cchar);
...
 mpc_ast_t * mpc_ast_add_child(mpc_ast_t * r, mpc_ast_t * a)
{
	...
	realloc(r->children, sizeof(mpc_ast_t *) * r->children_num);
...
 mpc_val_t * mpcf_fold_ast(int n, mpc_val_t ** xs)
{
	...
	return xs[1];
	    ...
	    continue;
			       ...
			       mpc_ast_add_root_tag(as[i]->children[0],
						    as[i]->tag));
...
 static mpc_val_t *mpcaf_grammar_string(mpc_val_t * x, void *s)
{
	...
	(st->flags & MPCA_LANG_WHITESPACE_SENSITIVE) ? mpc_string(y) :
...
 static mpc_val_t *mpcaf_grammar_char(mpc_val_t * x, void *s)
{
	...
	(st->flags & MPCA_LANG_WHITESPACE_SENSITIVE) ? mpc_char(y[0]) :
...
 static mpc_val_t *mpcaf_grammar_regex(mpc_val_t * x, void *s)
{
	...
	(st->flags & MPCA_LANG_WHITESPACE_SENSITIVE) ? mpc_re(y) :
	mpc_tok(mpc_re(y));
				  ...
				  mpca_grammar_st_t * st)
{
		  ...
		  mpc_and(2, mpcaf_grammar_or, Term,
			  mpc_maybe(mpc_and
				     (2, mpcf_snd_free, mpc_sym("|"),
				      Grammar, free)),
			  mpc_soft_delete ));
     mpc_define(Term, mpc_many1(mpcaf_grammar_and, Factor));
     mpc_define(Factor,
		  mpc_and(2, mpcaf_grammar_repeat, Base,
		  ...
		  mpc_or(5,
			 mpc_apply_to(mpc_tok(mpc_string_lit()),
				       mpcaf_grammar_string, st),
			 mpc_apply_to(mpc_tok(mpc_char_lit()),
				       mpcaf_grammar_char, st),
			 mpc_apply_to(mpc_tok(mpc_regex_lit()),
				       mpcaf_grammar_regex, st),
			 mpc_apply_to(mpc_tok_braces
				       (mpc_or
					(2, mpc_digits(), mpc_ident()),
					free), mpcaf_grammar_id, st),
	       ...
	       flags & MPCA_LANG_PREDICTIVE) ? mpc_predictive(r.output) :
	r.output;
 ...
 mpc_optimise_unretained(mpc_parser_t * p, int force)
{
	...
	/* Perform optimisations */ 
		...
		realloc(p->data.and.xs,
			sizeof(mpc_parser_t *) * (n + m - 1));
	    p->data.and.dxs =
		realloc(p->data.and.dxs,
