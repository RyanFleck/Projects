 ...
 * @author Lucia Moura
 * 
 * THIS DOCUMENT EDITED BY:
 * Ryan Fleck
 * sn 8276723
 * 
 * Completed on OCT 17, 2016.
 */
	  ...
	  while( bit.hasNext() )
		{
			...
			if( treeBit == 0 )
				{
					if( treeClone.left.isLeaf() )
						{
							return treeClone.left.getLetter();
						}
					treeClone = treeClone.left;
				}
			else if( treeBit == 1 )
				{
					if( treeClone.right.isLeaf() )
						{
							return treeClone.right.getLetter();
						}
					treeClone = treeClone.right;
				}
		}
	 return Integer.MAX_VALUE; /*** this needs to be changed since it is returning a                          wrong output (always code of special character EndOfText)***/
 }
